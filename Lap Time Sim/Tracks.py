...
def process_track(track_tuple, car, engine = "R6", track_name = "Default", save_drivecycle = False, verbose=False):    
    '''
    Runs a simulation of car around track_tuple, generated by read_track()
    The track_tuple is used to process a displacement vector of the track over whcih integration occurs to consider projected braking, acceleration zones,
    and corner handling (using car's cornering_speed() method). 

    Parameters:
    -----------
    track_tuple : tuple
        A tuple containing track data, typically including coordinates and other track-specific parameters.
    car : object
        An instance of the car class containing the car's parameters and powertrain.
    engine : str, optional
        The engine configuration to be used for the simulation. Default is "R6" - this will be undefined, and used for plotting and storing the Drive_Cycle data.
    track_name : str, optional
        The name of the track. Default is "Default", and is used to add info to the 'Drive cycle' plot for sake of data labelling.
    verbose : bool, optional
        If True, will show the plots.
    save_drivecycle : bool, optional
        If True, will save the drive cycle plot as a .png file, and in a .csv for further analysis.

    Adding the engine, track_name and drive_cycle as optional parameters means process_track() can be used the same as as for the Combustion engine (pre 2024), but with the additional parameters for the EV.

    Returns:
    --------
    tuple
        A tuple containing performance metrics such as lap time, energy consumption, and other relevant data.

    Notes:
    ------
    - The function assumes that the car object has methods and attributes required for the simulation.
    - The track_tuple should be formatted correctly, in a closed loop .dxf from CAD.
    - The function will show car velocities around the track in a colored 'speed-heat'-map, as well as a separate plot of the drive cycle if verbose is set to True.
    """'''
    global I_rms, V_max
    ...
    if verbose:
        ...
        # Drive_Cycle_Anchor 
        plt.suptitle(f"Duty cycle of vehicle comparing Velocity and Power plots over lap-time elapsed. \nEngine: {engine}, Track: {track_name}, V_max: {V_max}V, I_rms: {I_rms}A Pack assumption.")
        
        if save_drivecycle:
            # Visual plot
            name_precursor = f"DutyCycle_{engine}_{track_name}"
            plt.savefig(f"{name_precursor}.png")

            # Drive cycle plot into .csv
            filename = f"{name_precursor}.csv"
            with open(filename, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["Time (s)", "Velocity (m/s)", "Power (kW)", "Current (A)"])
                for t, v, p, c in zip(time_full, v_full, power_full, instant_current):
                    writer.writerow([t, v, p / 1000, c])
        plt.show()
